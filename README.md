...
裝好擴充即為完整的 Brume 介面——底層主題色（文字／引用／邊框／面板）由擴充
直接接管，**不需要**另外匯入或設定「介面主題」。
...
`themes/` 內附三個官方格式的介面主題檔（極光／水族館／夜霧），一般使用者
**可以無視**。只給兩種人：想在「使用者設定 → 介面主題」存一份原生 Brume
配色備用的人；或不裝擴充、只想要顏色（無背景動畫與泡泡版面）的人。
